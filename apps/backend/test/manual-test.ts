...
function testeNormal() {
  ...
  console.log(
    `- Usuários criados: ${cliente.getNome} (Cliente) e ${carpinteiro.getNome} (Prestador)`,
  );
  ...
  console.log(
    `- Serviço ofertado: ${servicoCama.getTitulo} | Preço: R$${servicoCama.getPrecoBase}`,
  );
  ...
  const valorFinal = contrato.calcularValorFinal();
  ...
  console.log(
    `- Fatura #${fatura.getIdFatura} gerada para ${fatura.getUsuario.getNome}`,
  );
...
function testarValidacoesDeIntegridade() {
  ...
  console.log(
    'Tentando criar usuário com nome inválido (menos de 3 caracteres)...',
  );
  try {
    new Usuario(
      'Jo',
    ...
    console.log(
      `✅ Erro esperado no construtor (Nome): ${error instanceof Error ? error.message : 'Erro desconhecido'}`,
    );
  }
  ...
  try {
    ...
    console.log(
      `✅ Erro esperado no setter (Nome): ${error instanceof Error ? error.message : 'Erro desconhecido'}`,
    );
  }
  ...
  try {
    ...
    console.log(
      `✅ Erro esperado no setter (Email): ${error instanceof Error ? error.message : 'Erro desconhecido'}`,
    );
  }
  ...
  try {
    ...
    console.log(
      `✅ Erro esperado no setter (CPF): ${error instanceof Error ? error.message : 'Erro desconhecido'}`,
    );
  }
  ...
  try {
    ...
    console.log(
      `✅ Erro esperado no construtor (CNPJ): ${error instanceof Error ? error.message : 'Erro desconhecido'}`,
    );
  }
}
...
function criarPersonagensBase(): { cliente: Usuario; carpinteiro: Prestador } {
  ...
  console.log(
    `- Prestador: ${carpinteiro.getNome} (ID: ${carpinteiro.getIdUsuario})`,
  );
  return { cliente, carpinteiro };
}

// Função 3: Prestador cadastra e gerencia serviços
function testarCadastroEGerenciamentoDeServico(
  carpinteiro: Prestador,
): Servico {
  ...
  console.log(
    `- Serviço cadastrado: "${servicoCama.getTitulo}" (ID: ${servicoCama.getIdServico})`,
  );
  console.log(
    `  Descrição: ${servicoCama.getDescricao} | Preço: R$${servicoCama.getPrecoBase}`,
  );

  servicoCama.setDescricao =
    'Cama de casal provençal, entalhada à mão, com acabamento em verniz fosco.';
  servicoCama.setPrecoBase = 1650.0;
  console.log(`- Serviço atualizado: "${servicoCama.getTitulo}"`);
  console.log(
    `  Nova Descrição: ${servicoCama.getDescricao} | Novo Preço: R$${servicoCama.getPrecoBase}`,
  );

  servicoCama.desativar();
  console.log(
    `- Status do serviço "${servicoCama.getTitulo}": ${servicoCama.getStatus}`,
  );
  servicoCama.ativar();
  console.log(
    `- Status do serviço "${servicoCama.getTitulo}": ${servicoCama.getStatus}`,
...
function testarFluxoDeIndicacao(
  cliente: Usuario,
  carpinteiro: Prestador,
): Indicacao {
  ...
  indicacao.registrarIndicacao();
  console.log(
    `- Indicação registrada (ID: ${indicacao.getIdIndicacao}) de ${indicacao.getIndicador.getNome} para ${indicacao.getIndicado.getNome}`,
  );
  console.log(`  Status inicial: ${indicacao.getStatusIndicacao}`);

  indicacao.setStatusIndicacao = StatusIndicado.ACEITA;
  console.log(
    `- Status da indicação atualizado para: ${indicacao.getStatusIndicacao}`,
  );
...
function testarContratacaoComDesconto(
  cliente: Usuario,
  servico: Servico,
  indicacao: Indicacao,
): Contratacao {
  ...
  console.log(
    `- Contratação solicitada (ID: ${contrato.getIdContratacao}) para "${servico.getTitulo}"`,
  );
  ...
  console.log(
    `- Indicação aplicada. Valor final com desconto (5%): R$${valorFinalComDesconto}`,
  );
...
function testarFaturamentoEFluxoDePagamento(
  ...
  const fatura = contrato.gerarFatura();
  console.log(
    `- Fatura (ID: ${fatura.getIdFatura}) gerada para ${cliente.getNome}`,
  );
  console.log(
    `  Valor da fatura: R$${fatura.getValorTotal} | Status: ${fatura.getStatusPagamento}`,
  );

  fatura.registrarPagamento(new Date(), FormaPagamento.PIX);
  console.log(
    `- Pagamento da fatura registrado. Novo status: ${fatura.getStatusPagamento}`,
  );
}

// Função 7: Testar registro financeiro do prestador
function testarRegistroFinanceiroDoPrestador(
  carpinteiro: Prestador,
  contrato: Contratacao,
  fatura: Fatura,
) {
  ...
  console.log(
    `- Receita registrada (ID: ${receita.getIdFinanceiro}): R$${receita.getValor} para contratação ${contrato.getIdContratacao}`,
  );
  ...
  console.log(
    `- Despesa registrada (ID: ${despesa.getIdFinanceiro}): R$${despesa.getValor} para contratação ${contrato.getIdContratacao}`,
  );
}
...
function testarAtualizacaoDePerfilDoUsuario(cliente: Usuario) {
  ...
  console.log(
    `- Perfil inicial de ${cliente.getNome}: Telefone ${cliente.getTelefone}, Endereço ${cliente.getEndereco}`,
  );
  cliente.setTelefone = '21912345678';
  cliente.setEndereco = 'Rua Nova, 100, Centro, Rio de Janeiro';
  console.log(
    `- Perfil atualizado: Telefone ${cliente.getTelefone}, Endereço ${cliente.getEndereco}`,
  );
}
...
function testarInativacaoDeConta(usuario: Usuario) {
  ...
  console.log(
    `- Status inicial da conta de ${usuario.getNome}: ${usuario.getStatusConta}`,
  );
...
function testarAvaliacaoDeServico(cliente: Usuario, contrato: Contratacao) {
  ...
  const avaliacao = new Avaliacao(
    contrato,
    4,
    'Ótimo serviço, muito profissional!',
  );
  avaliacao.registrar(4, 'Ótimo serviço, muito profissional!'); // O método registrar também valida
  console.log(
    `- Avaliação (ID: ${avaliacao.getIdAvaliacao}) registrada para o serviço da contratação ${contrato.getIdContratacao}`,
  );
  console.log(
    `  Nota: ${avaliacao.getNota} | Comentário: "${avaliacao.getComentario}"`,
  );

  avaliacao.setNota = 5;
  avaliacao.editarComentario('Serviço impecável, superou as expectativas!');
  console.log(
    `- Avaliação atualizada: Nota ${avaliacao.getNota} | Comentário: "${avaliacao.getComentario}"`,
  );
  ...
  try {
    ...
    console.log(
      `✅ Erro esperado no setter (Nota da Avaliação): ${error instanceof Error ? error.message : 'Erro desconhecido'}`,
    );
  }
}
...
function executarTestes() {
  console.log(
    '=== INICIANDO DEMONSTRAÇÃO DE CLASSES DE DOMÍNIO - PROJETO HIVE ===\n',
  );
  ...
  const contrato = testarContratacaoComDesconto(
    cliente,
    servicoCama,
    indicacao,
  );
  ...
  testarRegistroFinanceiroDoPrestador(
    carpinteiro,
    contrato,
    faturaParaFinanceiro,
  );
...
try {
  ...
  console.error('Erro inesperado durante a demonstração:', err);
  process.exitCode = 1;
}
