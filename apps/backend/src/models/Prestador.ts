...
export class Prestador extends Usuario {
  ...
  public cadastrarServico(
    ...
    console.log(
      `Cadastro de serviço "${novoServico.getTitulo}" gerada por ${this.nome}.`,
    );
    return novoServico;
  }

  public atualizarServico(servico: Servico, dados: Partial<Servico>): void {
  ...
  public registrarFinanceiro(
    ...
    console.log(
      `Registro financeiro de ${tipo} criado para a contratação #${contratacao.getIdContratacao}`,
    );
    return novaFinanca;
  }
  ...
  public get getAreaAtuacao(): string {
  ...
  public get getExperiencia(): string {
    return this.experiencia;
  }
  public set setExperiencia(exp: string) {
    if (exp.trim().length === 0)
      throw new Error('Descrição de experiência é obrigatória.');
    this.experiencia = exp.trim();
  }

  public get getCertificacoes(): string[] {
    return this.certificacoes;
  }
  public set setCertificacoes(certs: string[]) {
    if (!certs || certs.length === 0)
      throw new Error(
        'O prestador deve ter ao menos uma certificação ou habilidade listada.',
      );
    this.certificacoes = certs;
  }

  public get getAvaliacaoMedia(): number {
