...
export class Usuario {
  ...
  public cadastrar(): void {
    console.log(
      `Usuário ${this.nome} cadastrado com status ${this.statusConta}.`,
    );
  }
  ...
  public solicitarContratacao(
    ...
    console.log(
      `Contratação do serviço "${servico.getTitulo}" gerada por ${this.nome}.`,
    );
    return novaContratacao;
  }

  public pagarFatura(faturaId: number, metodo: FormaPagamento): void {
    console.log(`Fatura ${faturaId} paga via ${metodo}.`);
  }

  public receberIndicacao(indicacao: Indicacao): void {
    console.log(
      `Usuário ${this.nome} recebeu uma indicação através de ${indicacao.getMeioIndicado}.`,
    );
  }
  ...
  public set setSenha(senha: string) {
    if (senha.length < 8)
      throw new Error(
        'A senha deve conter no mínimo 8 caracteres por questões de segurança.',
      );
    this.senha = senha;
  }
  ...
  public set setEndereco(endereco: string) {
    if (endereco.trim().length < 5)
      throw new Error('Endereço muito curto ou incompleto.');
    this.endereco = endereco.trim();
  }

  public get getTipoUsuario(): TipoUsuario {
    return this.tipoUsuario;
  }
  public set setTipoUsuario(tipo: TipoUsuario) {
  ...
  public get getStatusConta(): StatusConta {
    return this.statusConta;
  }
  public set setStatusConta(status: StatusConta) {
