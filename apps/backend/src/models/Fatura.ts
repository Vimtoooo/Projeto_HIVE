...
export class Fatura {
  ...
  public gerar(): void {
    console.log(
      `Fatura ${this.idFatura} emitida para o usuário ${this.usuario.getNome}`,
    );
  }
  ...
  public registrarPagamento(data: Date, metodo: FormaPagamento): void {
    this.dataPagamento = data;
    this.statusPagamento = StatusPagamento.PAGO;

    console.log(
      `Pagamento de R$${this.valorTotal} registrado via ${metodo} em ${data.toISOString()}`,
    );
  }

  // Getters e Setters
  public get getIdFatura(): number {
  ...
  public set setUsuario(usuario: Usuario) {
  ...
  public get getFinanceiros(): Financeiro[] {
  ...
  public get getDataPagamento(): Date | null {
  ...
  public set setValorTotal(valor: number) {
    if (valor <= 0)
      throw new Error('O valor total da fatura deve ser positivo.');
    this.valorTotal = valor;
  }

  public get getStatusPagamento(): StatusPagamento {
    return this.statusPagamento;
  }
  public set setStatusPagamento(status: StatusPagamento) {
    this.statusPagamento = status;
  }
}
